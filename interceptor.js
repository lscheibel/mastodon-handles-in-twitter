 ...
 * ***/
{
    ...
    const extractUsersFromTweetResult = (tweetResult) => {
        ...
        if (retweetResult)
            extractUsersFromTweetResult(retweetResult);
    ...
    const userMentionsFediverse = (legacyUser) => {
    ...
    const findMastodonHandleInUrls = (urls) => {
        try {
    ...
    const validateHost = (host, strict = true) => {
        ...
        if (FORBIDDEN_HOSTS.includes(str))
            return false;
        if (!strict)
            return true;
    ...
    const getLinkedUrlsFromLegacyUser = (user) => {
        const urls = [];
        urls.push(...(user?.entities?.url?.urls?.map((u) => u.expanded_url) || []));
        urls.push(...(user?.entities?.description?.urls?.map((u) => u.expanded_url) || []));
        urls.push(...(user?.entities?.location?.urls?.map((u) => u.expanded_url) || []));
        return urls.filter(Boolean);
    };
    const mastodonHandleToInstanceUrl = (mastodonHandle) => {
        const [username, host] = mastodonHandle.split('@').filter(Boolean);
        return `https://${host}/@${username}`;
    };
    /*** Insert into UI ***/
    const idleInterval = (fn, t) => {
